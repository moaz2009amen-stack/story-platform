...
import {
  ...
  RiTranslateLine, RiHome2Line, RiArrowGoBackLine,
  RiHeartLine, RiHeartFill,
} from 'react-icons/ri'
import { storyHelpers, readingHelpers } from '../lib/supabase'
import { useAuth } from '../App'

/* ── Ending screen ──────────────────────────── */
function EndingScreen({ scene, lang, onRestart, story }) {
...
export default function StoryPlayer() {
  ...
  useEffect(() => {
    ...
    const visitedLen  = history.length + 1
    const progress    = Math.min(100, Math.round((visitedLen / totalScenes) * 100))

    localStorage.setItem(storageKey, JSON.stringify({ currentId, history, progress }))

    if (user) {
      readingHelpers.saveProgress(user.id, storyId, currentId, progress).catch(() => {})
    }
  }, [currentId, history])
  ...
  const author = story?.author?.full_name || story?.author?.username || ''
  ...
  return (
    <div style={{ background: 'var(--bg-base)', minHeight: '100vh' }}>
      ...
      {/* ── Top Bar ─────────────────────────── */}
      <div className="sticky top-0 z-40 transition-all" style={{ background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border-subtle)' }}>
          ...
          <div className="flex items-center gap-3 h-14">
            ...
            <div className="flex-1 min-w-0">
              ...
              {author && (
                <p className="text-xs flex items-center gap-1" style={{ color: 'var(--text-muted)' }}>
                  <RiUser3Line />{author}
                  {story?.reading_time && <><RiTimeLine className="mr-2" />{story.reading_time} دقائق</>}
                </p>
              )}
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button onClick={restart} className="btn-ghost w-9 h-9 p-0 rounded-xl"
                style={{ color: 'var(--text-muted)' }} title="إعادة">
                <RiRefreshLine />
              </button>
              <button
                onClick={() => {
              ...
              <button onClick={() => setLiked(l => !l)} className="btn-ghost w-9 h-9 p-0 rounded-xl">
      ...
      <div className="page-container py-8 md:py-12">
        <div className="max-w-2xl mx-auto">
          ...
          {currentScene?.isEnd ? (
            <EndingScreen scene={currentScene} lang={lang} onRestart={restart} story={story} />
          ) : (
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* Scene content */}
                <div
                  className="prose-story mb-10"
                  style={{ direction: lang === 'ar' ? 'rtl' : 'ltr' }}
                  dangerouslySetInnerHTML={{
                    __html: (currentScene?.content?.[lang] || currentScene?.content?.ar || '')
                      .replace(/\n/g, '<br />')
                  }}
                />
                    ...
                    <div className="space-y-3">
                      {currentScene.choices.map((choice, idx) => {
                        ...
                        return (
                          ...
                          >
                            <div className="w-8 h-8 rounded-xl flex items-center justify-center text-sm font-black shrink-0 transition-all"
          ...
          {history.length > 0 && (
            <div className="flex items-center gap-2 mt-12 pt-6" style={{ borderTop: '1px solid var(--border-subtle)' }}>
              <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
                المشهد {history.length + 1} من ~{totalScenes}
              </span>
