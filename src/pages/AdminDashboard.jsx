...
import { storyHelpers, profileHelpers } from '../lib/supabase'
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        {trend !== undefined && (
          <span className={`flex items-center gap-0.5 text-xs font-bold ${trend >= 0 ? 'text-green-500' : 'text-red-400'}`}>
            {trend >= 0 ? <RiArrowUpLine /> : <RiArrowDownLine />}{Math.abs(trend)}%
          </span>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6"
      ...
      <motion.div
        ...
        className="card-flat p-10 w-full max-w-sm text-center"
      >
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="relative">
            ...
            <input
              ...
              className="input-base pr-10 text-center"
              dir="ltr"
              autoFocus
      ...
      <div className="card-flat p-6">
        ...
        <div className="space-y-3">
          {recent.map((story, i) => {
            ...
            return (
              <div key={story.id} className="flex items-center gap-4 py-2.5 border-b last:border-0"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                    {story.author?.full_name || 'مجهول'} · {new Date(story.created_at).toLocaleDateString('ar-EG')}
                  </p>
                </div>
...
function StoriesTab({ stories, onTogglePublish, onDelete }) {
  ...
  const filtered = stories.filter(s => {
    ...
    if (search && !t.includes(q)) return false
    ...
    <div className="space-y-5">
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="بحث عن قصة..." value={search}
            onChange={e => setSearch(e.target.value)} className="input-base pr-10 text-sm" />
        </div>
        {['all', 'published', 'draft'].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            ...
            style={{
              ...
              color: filter === f ? '#0f0d0a' : 'var(--text-secondary)',
              ...
              <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0"
                ...
                {story.cover_image
                  ...
                  : <div className="w-full h-full flex items-center justify-center">
                      <RiBookOpenLine style={{ color: 'var(--text-muted)', opacity: 0.3 }} />
                    </div>
                }
              </div>
              <div className="flex-1 min-w-0">
...
function UsersTab({ users }) {
  ...
  return (
    <div className="space-y-5">
      <div className="relative">
        <RiSearchLine className="absolute right-3.5 top-1/2 -translate-y-1/2" style={{ color: 'var(--text-muted)' }} />
        <input type="text" placeholder="بحث عن مستخدم..." value={search}
          onChange={e => setSearch(e.target.value)} className="input-base pr-10 text-sm" />
      </div>
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-subtle)' }}>
              {['المستخدم', 'الصلاحية', 'تاريخ الانضمام'].map(h => (
                <th key={h} className="text-right px-5 py-3 font-semibold text-xs"
...
function AnalyticsTab({ stories }) {
  ...
  const catCount = stories.reduce((acc, s) => {
    const c = s.category || 'أخرى'
    acc[c] = (acc[c] || 0) + 1
    return acc
  }, {})
              ...
              <div key={story.id}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs font-bold" style={{ color: '#f59e0b' }}>
                    {(story.views || 0).toLocaleString('ar-EG')}
                  </span>
                </div>
            ...
            {topLiked.map((story, i) => (
              <div key={story.id} className="flex items-center gap-3">
                ...
                <span className="flex-1 text-sm truncate" style={{ color: 'var(--text-secondary)' }}>
                  {story.title?.ar || story.title || 'بلا عنوان'}
                </span>
                ...
                <div className="flex items-center gap-2">
                  <div className="w-20 h-1.5 rounded-full" style={{ background: 'var(--bg-subtle)' }}>
                    <div className="h-full rounded-full"
                      style={{ width: `${Math.round((count / stories.length) * 100)}%`, background: '#f59e0b' }} />
                  </div>
    ...
    <div className="space-y-6 max-w-lg">
      <div className="card-flat p-6">
        ...
        <form onSubmit={savePw} className="space-y-4">
          <div>
            <label className="label-sm">كلمة المرور الجديدة</label>
            <input type="password" value={newPw} onChange={e => setNewPw(e.target.value)}
              placeholder="••••••••" className="input-base mt-1 text-sm" dir="ltr" />
          </div>
          <div>
            <label className="label-sm">تأكيد كلمة المرور</label>
          ...
          <button type="submit" className="btn-primary">
            {pwSaved ? <><RiCheckLine /> تم الحفظ</> : 'حفظ كلمة المرور'}
          </button>
        </form>
      </div>
      ...
      <div className="card-flat p-6">
        ...
        <button onClick={onLock} className="btn-danger flex items-center gap-2">
          <RiCloseLine /> خروج
        </button>
      </div>
    </div>
...
export default function AdminDashboard() {
  ...
  if (!unlocked) return <LoginGate onUnlock={unlock} />
          ...
          <div className="flex items-center justify-between h-14">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-xs mr-2" style={{ color: 'var(--text-muted)' }}>قصة واختار</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <a href="/" className="btn-ghost text-xs px-3" style={{ color: 'var(--text-muted)' }}>
                ← الموقع
              </a>
            </div>
          </div>
